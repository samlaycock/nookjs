...
const samples: { name: string; code: string; iterations: number }[] = [
  ...
  {
    name: "Control Flow",
    code: "let sum = 0; for (let i = 0; i < 10; i++) { if (i % 2 === 0) { sum = sum + i; } } sum",
    iterations: 5000,
  },
  {
    name: "Functions + Closures",
    code: "function makeAdder(x) { return y => x + y; } let add10 = makeAdder(10); add10(5)",
    iterations: 5000,
  },
  {
    name: "Objects + Destructuring",
    code: "let obj = { a: 1, b: 2, c: { d: 4 } }; let { a, c: { d } } = obj; a + d",
    iterations: 5000,
  },
  {
    name: "Classes + Fields",
...
const results: ParseBenchmarkResult[] = samples.map((s) =>
  runParseBenchmark(s.name, s.code, s.iterations),
);
